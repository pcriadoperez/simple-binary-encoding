...
/**
 * {@link OutputManager} for managing the creation of PHP source files as the target of code generation.
 * The character encoding for the {@link java.io.Writer} is UTF-8.
 */
public class PHPOutputManager implements OutputManager
{
    private final File outputDir;

    /**
     * Create a new {@link OutputManager} for generating PHP source files into a given package.
     *
     ...
     */
    public PHPOutputManager(final String baseDirName, final String namespaceName)
    {
        ...
        final String packageDirName = dirName + namespaceName.replace('.', '_').replace(' ', '_').replace('-', '_');
    ...
    /**
     * Create a new output which will be a PHP source file in the given package.
     * <p>
     ...
     *
     * @param name the name of the PHP class.
    ...
    {
        final File targetFile = new File(outputDir, name + ".php");
